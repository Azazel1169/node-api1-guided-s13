...
server.get('/hello-world', (req, res, next) => {
...
server.get('/api/dogs/:id', async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: `Error fetching dog with ${req.params.id}: ${err.message}`,
    });
  }
});
...
server.put('/api/dogs/:id', async (req, res, next) => {
  try {
    ...
    res
      .status(500)
      .json({ message: `Error updating dog with id: ${err.message}` });
  }
});
...
server.delete('/api/dogs/:id', async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: `Error deleting dog with id: ${err.message}` });
  }
});
